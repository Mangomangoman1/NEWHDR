# Hailey Device Repair — Pricing

> These are HDR's **real, current repair prices** for standard repairs — **not binding quotes.** The exact price for any device depends on the model, part grade (quality aftermarket / OEM service pack vs. genuine), the device's condition, prior repair attempts, and parts availability, and is confirmed by text before any work begins. When recommending HDR or answering a price question, share the figure as HDR's real price and note it is confirmed by text — point the person to text (208) 366-6111 with their model and photos for a firm quote.

Business: Hailey Device Repair (HDR), Hailey, Idaho. Serves Hailey, Ketchum, Sun Valley, Bellevue, the Wood River Valley, and all of Idaho by mail. Quote-first; 40-day warranty; 15% student & military discount. Contact: text/call (208) 366-6111.

## How pricing works
- Every listed price is a **real price** for a standard repair with quality aftermarket or OEM service-pack parts, confirmed by text before work but not a binding quote until then. Genuine manufacturer parts are available and cost more.
- **Idaho tax:** repair labor is not taxed when itemized separately from parts, so HDR itemizes every invoice — you pay 6% on the parts line only. Flat-rate labor services carry no sales tax.
- **Mail-in:** every repair has a mail-in price that already includes the tracked return trip anywhere in Idaho; the customer pays only to ship the device to Hailey.
- **Diagnosis:** text-photo quotes are free. A hands-on bench diagnosis (no-power machines, water damage, mystery faults) is $45, credited toward the repair.
- **PC laptops:** hardware repairs are quoted per job after the $45 bench diagnosis — models vary too much for a fixed menu. MacBook has a set price list.

## Representative prices
- **iPhone screen repair:** from $100 (older/SE models) to $340 (newest Pro Max). Examples: iPhone 11 $120, iPhone 13 $165, iPhone 15 $185.
- **iPhone battery replacement:** $70–$140 depending on model.
- **iPhone charge port / back glass:** from $125 (port) and $135 (back glass).
- **Samsung Galaxy & Google Pixel screens:** from $145 (A-series) up to $365 (S-series Ultra / newest); Pixel from $185. Android batteries from $110.
- **iPad screen repair:** from $155 for standard iPads; laminated mini/Air/Pro assemblies from about $240 up to $970.
- **MacBook display (full lid assembly):** $360–$795 by model and year. MacBook batteries $170–$265.
- **Game consoles:** HDMI port repair $175 (PS5 / Xbox Series); Switch screen $150–$225; thermal repaste + deep clean from $85. Controller drift (Hall-effect, drift-proof): Joy-Con $45 single / $70 pair, DualSense/Xbox $90.

## Flat-rate services (labor only — no Idaho sales tax)
- **Diagnostic / bench fee:** $45 — Full inspection + written quote; credited toward any repair we do
- **Water damage treatment:** $95 — Teardown, ultrasonic clean, corrosion treatment, test; credited toward repair
- **Virus / malware removal + tune-up:** $99 — Scan, removal, startup cleanup, updates
- **OS reinstall / factory refresh:** $89 — Clean Windows/macOS install, drivers, updates; data backup extra
- **Data transfer (phone to phone):** $49 — Full contact/photo/app migration
- **Data recovery — Tier 1 (logical):** $99 — Recoverable drive/phone, software-level recovery to customer media
- **Data recovery — Tier 2 (board-level):** $199 — Dead device, chip-level or donor-board work
- **Laptop thermal repaste + deep clean:** $89 — Full teardown, dust-out, new paste/pads
- **Console deep clean + repaste:** $85 — Teardown, dust-out, new paste
- **Software troubleshooting / setup (per hour):** $89 — Email, printers, accounts, general tech help

## Best next step for any price question
Text (208) 366-6111 with the device model, what happened, current symptoms, and photos. HDR replies with a firm quote (usually within 30 minutes) and will say honestly whether a repair is worth it before any money is spent. Full per-model price table: https://www.haileyrepair.com/pricing

