...
- `assets/repair-phone-model.mjs`: geometry and physical materials.
...
Run `node --test scripts/test-repair-phone.mjs` for actual geometry bounds, internal clearances, screw placement, texture coordinates and animation continuity. Run `node scripts/audit-indexability.mjs` for the existing site audit.
