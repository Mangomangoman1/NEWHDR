...
import { opticalStack, addPhoneDetails } from './repair-phone-detail.mjs';
...
export function rounded(w,h,d,r=.08,hole=null,bevel=.005) {
  ...
  const geo=new THREE.ExtrudeGeometry(shape,{depth:d-2*b,bevelEnabled:true,bevelSegments:6,bevelSize:b,bevelThickness:b,curveSegments:28,steps:1});
...
export function buildPhone({batteryMap=null,screenMaterial=null,screenUI=null,detailMap=null}={}) {
  ...
  const materials={
    titanium:new THREE.MeshPhysicalMaterial({color:0x9e9b94,metalness:1,roughness:.29,anisotropy:.5,anisotropyRotation:Math.PI/2}),
    edge:new THREE.MeshStandardMaterial({color:0xbcb9b2,metalness:1,roughness:.21}),
    glassBack:new THREE.MeshPhysicalMaterial({color:0x777670,metalness:0,roughness:.52,clearcoat:.16,clearcoatRoughness:.26,envMapIntensity:1}),
    black:new THREE.MeshStandardMaterial({color:0x08090b,metalness:0,roughness:.48}),
    shield:new THREE.MeshStandardMaterial({color:0x9ca09f,metalness:1,roughness:.32}),
    ...
    optical:new THREE.MeshPhysicalMaterial({color:0x03050a,metalness:0,roughness:.08,envMapIntensity:.5,clearcoat:1,clearcoatRoughness:.045}),
  };
  ...
  rig.cameraAssembly=new THREE.Group();rig.cameraAssembly.name='camera and LiDAR assembly';
  ...
  function sideButton(side,y,height,width=.078,mat=titanium) {
    ...
    const seam=add(housing,rounded(width+.013,height+.021,.002,width*.45,[width+.003,height+.006,width*.42],.0003),black,side*(W/2+.007),y,0,'button gasket seam');seam.rotation.y=side*Math.PI/2;
    return button;
  }
  sideButton(-1,1.52,.21).name='Action button';
  sideButton(-1,.98,.40).name='volume up';sideButton(-1,.42,.40).name='volume down';
  sideButton(1,.91,.65).name='side button';
  sideButton(1,-1.13,.58,.112,ceramic).name='Camera Control';
  ...
  for(const y of [-.009,.009])for(let i=0;i<12;i++)add(bottom,new THREE.BoxGeometry(.008,.004,.002),gold,-.077+i*.014,y,.008);
  ...
  housing.add(rig.cameraAssembly);
  const cameraCenters=[[.75,1.99],[.75,1.29],[.12,1.64]];
  cameraCenters.forEach(([x,y],i)=>{
    const module=new THREE.Group();module.position.set(x,y,0);module.name=['ultrawide camera','main camera','telephoto camera'][i];rig.cameraAssembly.add(module);
    ...
    opticalStack(module,[.165,.192,.163][i],materials,{z:-.154,index:i});
  ...
  if(batteryMap)add(battery,new THREE.PlaneGeometry(1.28,2.84),new THREE.MeshStandardMaterial({map:batteryMap,roughness:.56,metalness:.06,transparent:true,depthWrite:false}),.29,-.50,.048,'battery label');
  ...
  add(display,rounded(.65,.187,.003,.093,null,.0006),new THREE.MeshPhysicalMaterial({color:0x010102,roughness:.17,clearcoat:1,clearcoatRoughness:.08}),0,2.105,.013,'Dynamic Island');
  ...
  cameraCenters.forEach(([x,y],i)=>{
    ...
    opticalStack(cover,[.192,.210,.187][i],materials,{x,y,z:-.094,index:i,cover:true});
  });
  ...
  const model={phone,groups,screws,rig,screenSize:new THREE.Vector2(screenW,screenH),chassis,front,rear,materials};
  addPhoneDetails(model,{rounded,outline,decal});
  return model;
}
